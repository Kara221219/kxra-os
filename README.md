...
`npm run test:ci` creates a fresh random-port PostgreSQL/two-application runtime, rejects a stale decoy service, applies and seeds 57 migrations, then runs formatting/type and lockfile policy checks, 130 database/domain/HTTP/security tests, a 153-table RLS audit, 42 private-OS and 12 public-site desktop/mobile browser scenarios, database/object restart and empty-target recovery, independent optimized production builds, compressed page-asset budgets, public/private source and exact snapshot verification, fixture/private-marker exclusion and a 295-file publication/secret scan. It stops the disposable database even on failure. GitHub Actions runs the same contract and pinned dependency audits; a separate SHA-pinned CodeQL workflow runs security-extended JavaScript/TypeScript analysis.
...
- Transaction-scoped PostgreSQL RLS across all 153 tables. Request bodies, headers, JWT organization metadata and model output cannot assign identity, tenant, role, project or approval authority.
