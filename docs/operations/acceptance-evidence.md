...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run test:ci`                      | **PASS** — 122/122 database/domain/HTTP tests; 57-migration/153-table audit; 38 applicable private-OS browser passes with 4 intentional skips plus 10/10 public-site passes; restart; both production builds; snapshot/source/artifact and 279-file publication scans |
...
| AT-28 hermetic CI                    | **PARTIAL / local PASS**   | A pinned GitHub workflow and `scripts/ci.mjs` create random-port disposable PostgreSQL and both application services, then run lint, 122 SQL/domain/HTTP tests, migration/RLS audit, 52 browser scenarios, snapshot/source checks, database/object restart, both clean production builds and artifact/publication scans. External AI evals, broader accessibility/security tooling and hosted Slice 8 workflow evidence remain later work. |
...
### AT-44 — Layered marketing accessibility: PASS (local subset)
...
- Ten desktop/mobile scenarios cover all routes, form completion, reduced motion, 320 px reflow, 200% text, keyboard-first skip navigation and no-JavaScript content/contact fallback.
- Reduced motion removes sticky and scroll-linked movement while preserving all four planes. Mobile recomposes cards/planes and has no horizontal overflow.
- Production-like Lighthouse/Core Web Vitals and representative screen-reader evidence remain required before release.
...
Slice 8's local public application and ingress boundary are complete within the limits above. The next evidence gate is Final Milestone 10 production quality: safe telemetry, CSP/edge/rate hardening, SAST/dependency gates, empty-target backup/restore and production-like accessibility/performance evidence. No external send, provider connection, candidate-code execution, merge, production deploy or publication is authorized.
