...
Review date: 24 September 2026. Branch: `codex/phase-2-completion`. Slice 6 parent baseline: `b39f9006183f2c49b27c15408f8ea23edea9cf72`. Requirements-freeze baseline: `4e597ea2039f8758a254cf42637baff26e7067a2`. Final Milestone 1 evidence: `08ac3d25f1f4127f617d91a41fa7dd353565e98b`.
...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run test:ci`                      | **PASS** — clean disposable PostgreSQL/application runtime; 114/114 database/domain/HTTP tests; migration audit; 36 applicable browser passes with 4 intentional skips; database/object restart; clean production build; artifact and publication scans |
| `npm run test:migrations`              | **PASS** — 53 contiguous migrations, 143 RLS tables with explicit policies, and non-superuser/non-bypass application/worker roles                                                                                                                      |
...
| `npm run test:secrets`                 | **PASS** — 230 tracked or untracked nonignored publication candidates checked against private paths and six credential patterns                                                                                                                     |
...
| --------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| AT-01 full access matrix          | **PASS (local)**            | A clean disposable run covers owner, contributor, viewer, revoked, onboarding, suspended, anonymous, other-organization, customer-admin and dual-membership cases across all 143 RLS tables. All tables have explicit policies; security tests enumerate all 118 exposed functions. HTTP tests cover current private route families, crafted tenant/project/resource IDs, cross-project Brand Studio/YouTube/repository/files/chunks/search/Ask/Ideas/workspaces/routines and owner-only routes. | Every future table/function/route must extend the matrix. Hosted pooler/grants remain untested.                                         |
...
| AT-18 usability/recovery          | **BLOCKED**                 | Disposable CI executes 36 applicable desktop/mobile scenarios with 4 intentional device-specific skips. Upload, indexed status, proxy download, chunk citation, local model synthesis, typed AI/routine registry inspection and the full Brand Studio source-to-export journey pass on both viewports. Existing surfaces reflow at 1440/768/390/320 and 200%; database/object restart integrity is automated.                                                         | Full product-wide state/WCAG/failure matrix and database-plus-object empty-target restore remain absent.                                |
...
Evidence: migrations `0031`, `0033`, `0035` and `0036`; `tests/phase2-commercial.test.ts`, `tests/phase2-http.test.ts`, `tests/e2e/phase2-identity-legal.spec.ts` and the complete current 143-table access matrix.
...
- The full current 143-table RLS matrix and 118-function exposure audit include all new objects.
...
The [account threat model](../security/account-identity-threat-model.md), [file/knowledge threat model](../security/file-knowledge-threat-model.md), [AI execution threat model](../security/ai-execution-threat-model.md), [Brand Studio threat model](../security/brand-studio-threat-model.md), [YouTube/repository threat model](../security/youtube-and-repository-pipelines-threat-model.md), [routine threat model](../security/routine-engine-threat-model.md) and [access-control model](../security/access-control.md) record assets, trust boundaries, attacks, controls and hosted gaps. Owner partner administration, the control plane, project workspaces, Brand Studio and Routine Registry collapse before tablet width; mobile navigation uses a keyboard-operable disclosure model. Automated inspection found no document-level overflow at the required frozen widths or 200% reflow.
...
Slice 6's governed local routine contract is complete within the limits above. The next evidence gate is the disabled-provider WhatsApp identity-pairing, durable ingress, explicit project selection, media quarantine and outbound-intent contract. No external send, provider connection, candidate-code execution, merge, production deploy or publication is authorized.
