...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run test:ci`                      | **PASS** — 228-package dependency policy; 131/131 database/domain/HTTP/security tests; 57-migration/153-table audit; 38 applicable private-OS browser passes with 4 intentional skips plus 14/14 public-site passes under development and optimized production; restart and empty-target restore; both production builds, exact CSP/SRI, compressed page-asset and optimized Lighthouse budgets; snapshot/source/artifact and 300-file publication scans |
...
| AT-18 usability/recovery          | **PARTIAL / local PASS**    | Disposable CI targets 38 applicable private and 14 public desktop/mobile scenarios with 4 intentional device-specific skips. Existing surfaces reflow at 1440/768/390/320 and 200%; public loading/503/input-preservation/retry succeeds; restart and empty-target database/object recovery are automated. | Full product-wide state/WCAG/failure matrix and hosted recovery evidence remain absent. |
...
| AT-27 telemetry/security controls    | **PARTIAL / local PASS**   | A closed telemetry envelope permits only bounded opaque operational fields. The dynamic OS has a request-nonce strict script CSP. Prerendered marketing uses 62 exact inline hashes, 122 SRI script references and no production script unsafe-inline; optimized desktop/mobile hydration and forms pass. A disconnected outbound-source contract revalidates URL/DNS/redirects, requires pinned addresses and caps response type/size/time. PostHog/Sentry capture, edge/storage/scanner controls and the complete staging matrix remain pending. |
| AT-28 hermetic CI                    | **PARTIAL / remote PASS**  | A pinned GitHub workflow and `scripts/ci.mjs` create random-port disposable PostgreSQL and both application services, reject a stale decoy, enforce lockfile policy, then run lint, SQL/domain/HTTP/security tests, migration/RLS audit, 42 private browser runs and 12 public scenarios twice, snapshot/source checks, restart/empty-target recovery, both builds, exact-hash/SRI and asset budgets plus artifact/publication scans. Full CI run 36253210907 and SHA-pinned CodeQL run 36253210903 passed for Slice 13 commit `a10a4bd`; the CSP production-server extension passes locally. Hosted providers and external AI evaluations remain pending. |
...
- Hosted identity accepts only one valid `x-vercel-forwarded-for` address when `VERCEL=1`, ignores a conflicting caller `x-forwarded-for`, rejects lists/malformed/missing addresses and has no unreviewed hosted fallback. Loopback fixtures have a separate explicit path.
...
- Optimized mobile and desktop Lighthouse audits enforce performance at least 0.90, accessibility 1.00, LCP at most 2.5 seconds, CLS at most 0.1 and TBT at most 200 ms. Accepted measurements were mobile home 1.00/1.00, 1,899 ms LCP, 0.000 CLS and 41 ms TBT; desktop contact 1.00/1.00, 427 ms, 0.000 and 0 ms.
- TBT is a laboratory proxy for INP. Field Web Vitals and human assistive-technology review remain required before release.
