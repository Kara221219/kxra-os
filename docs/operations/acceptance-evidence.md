...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run test:ci`                      | **PASS** — 118-package dependency policy; 129/129 database/domain/HTTP/security tests; 57-migration/153-table audit; 38 applicable private-OS browser passes with 4 intentional skips plus 10/10 public-site passes; restart and empty-target restore; both production builds; snapshot/source/artifact and 294-file publication scans |
...
| AT-28 hermetic CI                    | **PARTIAL / local PASS**   | A pinned GitHub workflow and `scripts/ci.mjs` create random-port disposable PostgreSQL and both application services, reject a stale decoy, enforce lockfile source/integrity/license/install-script policy, then run lint, 129 SQL/domain/HTTP/security tests, migration/RLS audit, 52 browser scenarios, snapshot/source checks, restart/empty-target recovery, both builds and artifact/publication scans. A SHA-pinned CodeQL security-extended workflow is present; its first remote result is pending. |
