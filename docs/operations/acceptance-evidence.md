...
Review date: 25 September 2026. Branch: `codex/phase-2-completion`. Slice 7 parent baseline: `7fc2a34d01d42fa078a15d8ef526d428a4dde717`. Requirements-freeze baseline: `4e597ea2039f8758a254cf42637baff26e7067a2`. Final Milestone 1 evidence: `08ac3d25f1f4127f617d91a41fa7dd353565e98b`.
...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run test:ci`                      | **PASS** — 119/119 database/domain/HTTP tests; 55-migration/151-table audit; 38 applicable browser passes with 4 intentional skips; restart; production build; artifact and 237-file publication scans |
| `npm run test:migrations`              | **PASS** — 55 contiguous migrations, 151 RLS tables with explicit policies, and non-superuser/non-bypass application/worker roles                                                                                                                      |
...
| `npm run test:secrets`                 | **PASS** — 237 tracked or untracked nonignored publication candidates checked against private paths and six credential patterns                                                                                                                     |
...
| --------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| AT-01 full access matrix          | **PASS (local)**            | A clean disposable run covers owner, contributor, viewer, revoked, onboarding, suspended, anonymous, other-organization, customer-admin and dual-membership cases across all 151 RLS tables. All tables have explicit policies; security tests enumerate all 121 exposed functions. HTTP tests cover current private route families, crafted tenant/project/resource IDs, cross-project Brand Studio/YouTube/repository/files/chunks/search/Ask/Ideas/workspaces/routines/WhatsApp and owner-only routes. | Every future table/function/route must extend the matrix. Hosted pooler/grants remain untested.                                         |
...
| AT-18 usability/recovery          | **BLOCKED**                 | Disposable CI targets 38 applicable desktop/mobile scenarios with 4 intentional device-specific skips. Upload, indexed status, proxy download, chunk citation, local model synthesis, typed AI/routine/WhatsApp status inspection and the full Brand Studio source-to-export journey pass on both viewports. Existing surfaces reflow at 1440/768/390/320 and 200%; database/object restart integrity is automated.                                              | Full product-wide state/WCAG/failure matrix and database-plus-object empty-target restore remain absent.                                |
...
| AT-28 hermetic CI                    | **PARTIAL / local PASS**   | A pinned GitHub workflow and `scripts/ci.mjs` create random-port disposable PostgreSQL/application services and run lint, 119 SQL/domain/HTTP tests, migration/RLS audit, 42 browser scenarios, database/object restart, clean production build and artifact/publication scans. Full public/customer builds, external AI evals, broader accessibility/security tooling and hosted workflow evidence remain later work. |
...
Evidence: migrations `0031`, `0033`, `0035` and `0036`; `tests/phase2-commercial.test.ts`, `tests/phase2-http.test.ts`, `tests/e2e/phase2-identity-legal.spec.ts` and the complete current 151-table access matrix.
...
- The full current 151-table RLS matrix and 121-function exposure audit include all new objects.
...
Slice 7's local WhatsApp authority contract is complete within the limits above. The next evidence gate is the independent public build and approved-publication snapshot boundary. No external send, provider connection, candidate-code execution, merge, production deploy or publication is authorized.
