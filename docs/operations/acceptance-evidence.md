...
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run test:ci`                      | **PASS** — 124/124 database/domain/HTTP tests; 57-migration/153-table audit; 38 applicable private-OS browser passes with 4 intentional skips plus 10/10 public-site passes; restart and empty-target restore; both production builds; snapshot/source/artifact and 286-file publication scans |
...
| AT-27 telemetry/security controls    | **PARTIAL / local PASS**   | A closed telemetry envelope permits only bounded opaque operational fields and rejects identity, content, prompt, file, secret, nested and unknown data before capture. Existing worker, artifact and redaction controls remain green. PostHog/Sentry capture, CSP/edge/storage/scanner controls and the complete cross-cutting matrix are not configured. |
| AT-28 hermetic CI                    | **PARTIAL / local PASS**   | A pinned GitHub workflow and `scripts/ci.mjs` create random-port disposable PostgreSQL and both application services, reject a stale decoy through per-run readiness identity, then run lint, 124 SQL/domain/HTTP tests, migration/RLS audit, 52 browser scenarios, snapshot/source checks, restart/empty-target recovery, both clean production builds and artifact/publication scans. External AI evals and broader accessibility/security tooling remain later work. |
