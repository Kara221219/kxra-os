...
Status: Final Milestones 1–4 and Phase 2 Slices 0–8 are implemented in the deterministic local environment on `codex/phase-2-completion`. Multi-tenant identity, first-private-access legal gating, deterministic commercial/custom-project foundations, the private file/knowledge lifecycle, permission-safe local AI execution, KXRA Brand Studio, governed PROJECT-006/007 local pipelines, the governed local Routine Registry, the transport-disabled WhatsApp authority contract and the independent public marketing application now exist. Hosted providers and public deployment remain absent. The target architecture is in [Phase Completion Brief 02](../operations/CODEX-PHASE-COMPLETION-BRIEF-02.md); implemented decisions include [ADR 0008](../decisions/0008-multi-tenant-legal-commercial-foundation.md), [ADR 0009](../decisions/0009-secure-file-and-knowledge-lifecycle.md), [ADR 0010](../decisions/0010-permission-safe-ai-execution.md), [ADR 0011](../decisions/0011-kxra-brand-studio-local-product.md), [ADR 0012](../decisions/0012-governed-youtube-and-repository-pipelines.md), [ADR 0013](../decisions/0013-governed-routine-engine.md), [ADR 0014](../decisions/0014-whatsapp-gateway-authority.md) and [ADR 0015](../decisions/0015-independent-public-marketing-boundary.md). This document is not hosted or production evidence.
...
The application exposes 122 bounded functions to authenticated or anonymous roles. Tests enumerate every table and exposed function and fail if either grows without an authorization decision. Composite foreign keys bind organization/project scope. Typed security-definer functions validate context, legal, entitlement, usage, billing reconciliation, custom-project, file/knowledge, AI-run, Brand Studio, YouTube-content, repository-adoption, routine-control, WhatsApp pairing/project-selection and public-enquiry transitions; ordinary RLS controls reads. Dedicated file, AI, routine and WhatsApp worker roles receive only their private bounded functions. PROJECT-006 channel verification/disconnection remains private and PROJECT-007 exposes no worker or execution function.
...
`apps/marketing` is an independently buildable server-rendered application. Its only content authority is the hash-bound disabled snapshot under `apps/marketing/content`; a source audit rejects imports from `apps/os`, shared private packages, fixtures or Genesis material. Required current and preserved routes render without a private API. `/login` redirects to a server-configured private-application URL and requires HTTPS in production.
...
Vercel is the only approved hosted HTTP edge for public ingress. The application accepts one valid `x-vercel-forwarded-for` address only when running under Vercel, then immediately HMAC-digests it with the bounded user-agent input. Cloudflare remains DNS-only unless Vercel Trusted Proxy is separately purchased and proven. See [ADR 0019](../decisions/0019-vercel-trusted-public-ingress-edge.md).
